...
## Sending the form
You can use IDs to easily identify forms.

### Option 1: Random ID
If you do not need the ID - for instance, because you don't handle forms using the Event.
```java
form.send(player);
```

### Option 2: Specific ID
```java
form.send(player, 123);
```

## Updating forms
If you want to update a form *while it is still opened to the player*, you can edit any created buttons by changing its text or image or applying a new button:
